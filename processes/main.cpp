...
{//GetLastError это ОС зависимая функция, коды возврата могут не совпадать при слвпадении причины ошибки
	...
	if (!ret) { DWORD GLE = GetLastError(); if ((GLE == 3435973836) || (GLE == 1062)) return true; }/*3435973836 и 1062<--- это значения возможных ошибок, если произошла попытка остановить уже остановленную службу*/
	return ret;
}
